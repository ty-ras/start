...
await execution.invokeMain({
    ...
    getServerParameters: ({ http }) => {
        const environment = {
            tokenVerifier: () => TE.left(new Error("When creating authentication for the HTTP server, please replace this code call to actual token verification code in src/environment/<cloud provider name>/xyz.ts files.")),
        };
        ...
        return Promise.resolve({
            ...
            parameters: {
                ...
                getAdditionalStateValue: () => {
                    throw new Error("When there are other properties than resource pools or authentication properties, this callback will be responsible to provide those for endpoints.");
                },
            },
            admin: {
                // Needed so that
                allPoolAdmins: Object.values(poolsWithAdmins).map(({ administration }) => administration),
                environment,
            },
        });
    },
